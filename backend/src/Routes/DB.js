...
const API_KEY = process.env.MONGO_DB_URL;
let db
const DBStart = async() => {
...
const submitQuiz = async(submittedQuiz, res) => {
    withDB(async(db) => {
        try {
            ...
            await db.collection('quizzes').updateOne({ _id: new ObjectId(submittedQuiz.quizId) }, {
                    $push: {
                        responses: { uid: submittedQuiz.uid, score: score }
                    }
                })
                // Update user's attempted quizzes
            await db.collection('users').updateOne({ uid: submittedQuiz.uid }, {
                $push: {
                    attemptedQuiz: ObjectId(submittedQuiz.quizId)
                }
            })
